...
def main():
    ...
    with torch.no_grad():
        ...
        config = os.path.join(script_dir, 'wholebody_w48_384x288.yaml')
        ...
        config1 = os.path.join(script_dir, 'hrnet_w48_coco_wholebody_384x288-6e061c6a_20200922.pth')
        checkpoint = torch.load(config1)
        ...
        # Input file path (single video)
        input_path = '/home/Student/s4582342/data/test/M_00002_res.mp4'

        # Output file path (including filename)
        output_npy = '/home/Student/s4582342/wholebody_val/output.npy'

        # Check if the output directory exists, create it if needed
        output_directory = os.path.dirname(output_npy)
        if not os.path.exists(output_directory):
            os.makedirs(output_directory)

        # Check if the output file already exists
        if os.path.exists(output_npy):
            print("Output file already exists:", output_npy)
        else:
