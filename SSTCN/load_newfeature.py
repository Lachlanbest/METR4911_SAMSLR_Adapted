...
class TorchDataset(Dataset):
    ...
    repeat=1):
        self.load_name = '/home/Student/s4582342/CVPR21Chal-SLR/SSTCN/train_val_split.mat'
    ...
    def read_file(self):
        ...
        if self.istrain:
            for idx in range(self.train_number):
              name = self.train_file_name[idx][0][0][0][0] + '.pt'
              labels = self.train_label[idx][0]
              fea_label_list.append((name, labels))
              name = self.train_file_name[idx][0][0][0][0] + '_flip.pt'
              labels = self.train_label[idx][0]
              fea_label_list.append((name, labels))
        else:
            for idx in range(self.test_number):
              name = self.test_file_name[idx][0][0][0][0] + '.pt'
